...
class InstaAutomate:
    ...
    def like_posts_by_hashtags(self, hashtag_list, follow=False, comment=False):
        ...
        self.session.like_by_tags(hashtag_list, amount=10)
        if follow:
            print("MANUAL LOG-> FOLLOWING USERS")
            self.session.set_do_follow(True, percentage=80)
        if comment:
            print("MANUAL LOG-> COMMENTING ON USERS")
...
if __name__ == "__main__":
    ...
    ia.like_posts_by_hashtags(
        hashtag_list=['rakshabandhan', 'rakhi', 'rakhigift', 'rakhigifts'],
        follow=True,
        comment=True
    )